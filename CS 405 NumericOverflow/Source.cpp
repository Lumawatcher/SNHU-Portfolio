...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        ...
        // if a buffer overflow is going to happen, we set result to 0 and return
        if ((max - increment) < result)
        {
            result = 0;
            return result;
        }
...
void test_overflow()
{
    ...
    //  SOLUTION is on lines 104 - 114 and 26 - 32
    ...
    //CHANGED FOR ASSIGNMENT
    //result = 0 if buffer overflow would occur during add numbers
    if (result == 0)
    {
...
void test_underflow()
{
    ...
    //CHANGED FOR ASSIGNMENT
    //result = 0 if buffer underflow would occur during subtract numbers
    if (result == 0)
    {
