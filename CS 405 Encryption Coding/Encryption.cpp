...
std::string read_file(const std::string& filename)
{
  ...
  // DONE: implement loading the file into a string
    ...
    //Open an input file stream, and process an iterator over the stream.
    std::ifstream fileStream(filename);
    std::string file_text( (std::istreambuf_iterator<char>(fileStream) ),
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  //  Line 4+: data
    ...
    //Construct our new file string
    std::string fileText = student_name + "\n" + "2023/30/07" + "\n" + key + "\n" + data;
    //Open out file stream and attach a file stream iterator
    std::fstream fileStream(filename);
    std::ostream_iterator<char>outIterator(fileStream);

    //Copy the new tile text to the file stream
    std::copy(fileText.begin(), fileText.end(), outIterator);
}
...
int main()
{
  ...
  const std::string key = "JaredsKey";
