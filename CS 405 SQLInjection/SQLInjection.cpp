...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // CHANGED added input validation to prevent this type of SQLI
  // SQLI prevention: Invalid inputs
    std::string noOR = " or ";
    if (sql.find(noOR) != std::string::npos)
    {
        std::cout << "SQL query aborted, potential SQL injection detected. Please use a query without ' or '." << std::endl;
        return false;
    }
